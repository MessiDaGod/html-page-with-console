...
(function () {
    ...
    window.Shakely = {
        ...
        draw: function () {
            ...
            if (Shakely.b_drawMouseOverlay) {
                ...
                //     Shakely.fillRect(Shakely.mousePosition.x + 15, yPos, 100, 80, Shakely.canvas.style.backgroundColor);
                // }

                // Shakely.ctx.fillStyle = Shakely.gridTextColor;
                // Shakely.ctx.fillText("Open: " + Shakely.fmt(Shakely.candlesticks[Shakely.hoveredCandlestickID].open), Shakely.mousePosition.x + 17, yPos + 15);
                // Shakely.ctx.fillStyle = Shakely.gridTextColor;
                // Shakely.ctx.fillText("High: " + Shakely.fmt(Shakely.candlesticks[Shakely.hoveredCandlestickID].high), Shakely.mousePosition.x + 17, yPos + 35);
                // Shakely.ctx.fillStyle = Shakely.gridTextColor;
                // Shakely.ctx.fillText("Low: " + Shakely.fmt(Shakely.candlesticks[Shakely.hoveredCandlestickID].low), Shakely.mousePosition.x + 17, yPos + 55);
                // Shakely.ctx.fillStyle = Shakely.gridTextColor;
                // Shakely.ctx.fillText("Close: " + Shakely.fmt(Shakely.candlesticks[Shakely.hoveredCandlestickID].close), Shakely.mousePosition.x + 17, yPos + 75);

                // new Log(Shakely.mousePosition.y);
                //new Log(textWidth);

                // header.ctx.fillStyle = Shakely.gridTextColor;
                // header.ctx.fillText("H: " + Shakely.fmt(Shakely.candlesticks[Shakely.hoveredCandlestickID].high), 1100 + textWidth + 5, 20);
                // header.ctx.fillStyle = Shakely.gridTextColor;
                // header.ctx.fillText("L: " + Shakely.fmt(Shakely.candlesticks[Shakely.hoveredCandlestickID].low), 1100 + (textWidth * 2) + 5 , 20);
                // header.ctx.fillStyle = Shakely.gridTextColor;
        ...
        drawGrid: function () {
            ...
            for (var y = yStartRoundNumber; y <= yEndRoundNumber; y += niceNumber) {
                ...
                Shakely.ctx.fillText(Shakely.roundPriceValue(y), Shakely.width - textWidth - 5, Shakely.yToPixelCoords(y) - 5);
                Shakely.ctx.fillStyle = Shakely.gridTextColor;
                Shakely.ctx.fillText(Shakely.roundPriceValue(currentprice), Shakely.width - textWidth - 5, Shakely.yToPixelCoords(currentprice) - 5);
        ...
        getCandleSticks: function ( /** @type {string | URL} */ url) {
            this.webSocketConnected = false;
            this.webSocketHost = "wss://stream.binance.com:9443/ws/" + "BTCUSDT" + "@kline_" + "1";
            if (url == null) url = "https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1d&limit=100";
            var xmlhttp = new XMLHttpRequest();
            xmlhttp.open("GET", url, true);
            xmlhttp.onreadystatechange = function () {
                if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
    ...
    //     document.getElementsByTagName("p")[0].innerHTML += '<font color="grey">' + things[i] + '</font>' + " → " + colorize(eval(things[i])) + "<br><br>";
    // }
